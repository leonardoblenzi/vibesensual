...
const ProductManagementView: React.FC = () => {
  ...
  // Fix: Converted to async to await results from InventoryController
  const loadData = async () => {
    const prods = await InventoryController.getProducts();
    const cats = await InventoryController.getCategories();
    setProducts(prods);
    setCategories(cats);
  };
  ...
  // Fix: Added async/await for data persistence
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await InventoryController.saveProduct(productToSave);
    await loadData();
    setIsModalOpen(false);
  };

  // Fix: Added async/await for deletion
  const handleDelete = async (id: string) => {
    if (confirm('Deseja excluir este produto permanentemente?')) {
      await InventoryController.deleteProduct(id);
      await loadData();
    }
  };
